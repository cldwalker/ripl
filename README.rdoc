...
* Enhancements over irb
  ...
  * Less than 175 loc vs irb's 5000+ loc
...
A ripl plugin is a module that is included into Ripl::Shell or Ripl::Runner. As an example let's color error
messages red. Knowing that error messages are formatted by Ripl::Shell#format_error and terminal strings are
...
ripl highly encourages plugins by loading them as early as possible and allowing them to effect
almost any aspect of ripl. To customize the shell and the runner (for option and argument parsing),
see Ripl::Shell::API and Ripl::Runner::API.
